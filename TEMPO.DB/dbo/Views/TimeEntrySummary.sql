...
select entryid, projectid, sum(sunday+monday+tuesday+wednesday+thursday+friday+saturday) as entryHours, (sum(sunday+monday+tuesday+wednesday+thursday+friday+saturday) * employee.rate) as internalamount,
			employee.employeename, periodending.endingdate, timesheet.tid, worktype.worktypename, worktype.worktypeid
...
group by endingdate, projectid, worktypename, entryid, employee.rate, employeename, timesheet.tid, worktype.worktypeid